...
Axum has a limitation that you can only have _one_ state per `Router`. So now you face a choice:

1. You can group everything into a single `State` object and use careful interior mutability.
2. You can replace `State` with an _Extension Layer_.
...
> Tip: Use state if you only need one. Extensions aren't _that_ heavy, it will still perform very well.
...
> [Next](/07-RustService/simple_tower_recap.md)
