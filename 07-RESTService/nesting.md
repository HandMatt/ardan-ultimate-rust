...
For the parent `Router`, we call `.nest` with a _parent path_ and a function that returns a `Router`. These could be in different modules or crates. We also used an async closure to quickly build the response methods.

The parent path is a concatenation:

- Service one starts with a _base_ of `/1`---and then `/` is appended. The _result_ is `/1/`.
...
> [Next](/07-RESTService/nesting_state.md)
