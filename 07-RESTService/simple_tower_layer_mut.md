...
We're using the _interior mutability_ pattern we talked about it in Rust Foundations: the shared state remains immutable from the outside, but because it protects its mutable contents---it can be safely shared with an `Arc`.

Now you have the ability to share mutable and immutable state.

> [Next](/07-RESTService/simple_tower_layer_multi_state.md)
