...
Say you want to implement an API that responds to `/book/1` and `/book/2` by returning that book's data. This can be accomplished with a _path extractor_.

> The code for this is in `code/rest_service/axum_extract_path`.
...
- The handler declares `Path(id): Path<u32>` as a parameter.
  - `Path(id)` means "match the Path extractor, and name the result id".
    - `Path(..)` is actually _destructuring_. You can pattern match in function parameters.
...
> [Next](/07-RESTService/axum_extractors_query.md)
