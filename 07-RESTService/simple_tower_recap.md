...
- Routers can add `layer`s. The `Extension` layer serves as additional state, and can be used to share as many injectible dependencies as you want. State is _slightly_ faster than layers, but less flexible.
- Shared data in both _state_ and _extension layers_ must still obey Rust's safety rules. No data-races here!
...
> [Next](/07-RESTService/nesting.md)
